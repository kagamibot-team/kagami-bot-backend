...
class Award(Model, BaseMixin):
    ...
    description: Mapped[str] = mapped_column(
        default="这只小哥还没有描述，它只是静静地躺在这里，等待着别人给他下定义。"
    )
    ...
    binded_counters: Mapped[list["AwardCountStorage"]] = relationship(
        back_populates="target_award", lazy="subquery"
    )
    binded_stats: Mapped[list["AwardUsedStats"]] = relationship(
        back_populates="target_award", lazy="subquery"
    )
    binded_skins: Mapped[list["AwardSkin"]] = relationship(
        back_populates="applied_award", lazy="subquery"
    )
...
class AwardCountStorage(Model, BaseMixin):
    ...
    target_user: Mapped["UserData"] = relationship(
        back_populates="award_counters", lazy="subquery"
    )
    target_award: Mapped[Award] = relationship(
        back_populates="binded_counters", lazy="subquery"
    )
...
class AwardUsedStats(Model, BaseMixin):
    ...
    target_user: Mapped["UserData"] = relationship(
        back_populates="award_stats", lazy="subquery"
    )
    target_award: Mapped[Award] = relationship(
        back_populates="binded_stats", lazy="subquery"
    )
...
class UserData(Model, BaseMixin):
    ...
    award_counters: Mapped[list[AwardCountStorage]] = relationship(
        back_populates="target_user", lazy="subquery"
    )
    award_stats: Mapped[list[AwardUsedStats]] = relationship(
        back_populates="target_user", lazy="subquery"
    )
    ...
    used_skins: Mapped[list["SkinRecord"]] = relationship(
        back_populates="user", lazy="subquery"
    )
    owned_skins: Mapped[list["SkinOwnRecord"]] = relationship(
        back_populates="user", lazy="subquery"
    )
...
class SkinRecord(Model, BaseMixin):
    ...
    skin: Mapped["AwardSkin"] = relationship(
        back_populates="binded_records", lazy="subquery"
    )
...
class SkinOwnRecord(Model, BaseMixin):
    ...
    skin: Mapped["AwardSkin"] = relationship(
        back_populates="own_records", lazy="subquery"
    )
...
class AwardSkin(Model, BaseMixin):
    ...
    applied_award: Mapped[Award] = relationship(
        back_populates="binded_skins", lazy="subquery"
    )
    binded_records: Mapped[list[SkinRecord]] = relationship(
        back_populates="skin", lazy="subquery"
    )
    own_records: Mapped[list[SkinOwnRecord]] = relationship(
        back_populates="skin", lazy="subquery"
    )
