...
command_content: dict[str, HelpData] = {
    "抓小哥": HelpData(
        ...
        optional_parameters=HelpParam(note="", content=[
            HelpPair(key=r"{次数N}", value="有足够次数的情况下抓N次小哥，否则抓完目前积攒的所有次数。"),
        ]),
        related_commands=HelpParam(note="", content=[
    ...
    "展示": HelpData(
        ...
        required_parameters=HelpParam(note="", content=[
            HelpPair(key=r"{小哥名X}", value="指明需要展示信息的小哥X。"),
        ]),
        optional_parameters=HelpParam(note="（可叠加）", content=[
            HelpPair(key="-d", value="以抓取条目的形式展示该小哥。每个小哥左下白数字表示目前的存量，左上的黑数字表示获得过的总数。"),
    ...
    "合成": HelpData(
        ...
        required_parameters=HelpParam(note="", content=[
            HelpPair(key=r"{小哥名X1} {小哥名X2} {小哥名X3}", value="将X1、X2、X3三个小哥送入合成机器。"),
        ]),
        optional_parameters=None,
        related_commands=None,
    ),
    ...
    "切换皮肤": HelpData(
        ...
        required_parameters=HelpParam(note="(选其一)", content=[
    ...
    "小镜晚安": HelpData(
        ...
        required_parameters=None,
        optional_parameters=HelpParam(note="", content=[
            HelpPair(key=r"{时间点X}", value="自然语言写出的时间点（仅限整点或半点），如“明早八点”、“五点半”等。（留空默认为“八点”）"),
        ]),
        related_commands=None,
    ),
    "丢粑粑": HelpData(
    ...
    "抓小哥更新": HelpData(
        ...
        required_parameters=None,
        optional_parameters=HelpParam(note="", content=[
            HelpPair(key=r"{页码N}", value="展示更新信息的第N页。（留空默认为“1”）"),
        ]),
        related_commands=None,
    ),
    "关于小镜bot": HelpData(
